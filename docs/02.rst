...
AWSコンソールの操作は、お手持ちのPCで行って頂いても、Raspberry Piのリモート画面で実施して頂いても構いません。Raspberyy Piで実施する場合は、「Raspberry Piの設定」の章に従って、VNC Viewerを用いて、Raspberry Piにリモート接続して設定を実施して下さい。
